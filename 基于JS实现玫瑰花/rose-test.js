    ...
    if(A*A + B*B <1){
        if(c > 37){
            ...
            return{
                ...
                r : .4 - a * .1 + Math.pow(1-B*B,-h*6)*.15-a*b*.4+Math.cos(a+b)/5
                + Math.pow(Math.cos((o*(a+1)+(B>0?w:-w))/25),30)*.1*(1-B*B),
                g: o/1e3+.7 - o*w*3e-6
            };
        }
        if(c > 32){
            ...
            var o = a * 45 -20;
            var w = b * b *h;
            var z = o * Math.sin(c) + w*Math.sin(n) + 620;
            return{
        ...
        return {
            x : o*Math.cos(c) - w*Math.sin(c),
            y : B*99 - Math.cos(Math.pow(b,7))*50 - c/3 - z/1.35
            + 450,
            z : z,
            r : (1-b/1.2)*.9 + a*.1,
